...
def create_grouped_award_category_df(df, filter_column, award_column):
    ...
    grouped_filtered_df = filtered_df[['playerID', award_column]].groupby(['playerID'], dropna=False, as_index=False).sum()
    
    return grouped_filtered_df

def create_grouped_award_df(df):
    grouped_pitching_triple_crown_df = create_grouped_award_category_df(df, 'Pitching Triple Crown', 'PitchingTripleCrowns')
    
    grouped_triple_crown_df = create_grouped_award_category_df(df, 'Triple Crown', 'TripleCrowns')
    
    grouped_mvp_df = create_grouped_award_category_df(df, 'Most Valuable Player', 'MVPs')
    
    grouped_roy_df = create_grouped_award_category_df(df, 'Rookie of the Year', 'ROYs')
    
    grouped_ws_mvp_df = create_grouped_award_category_df(df, 'World Series MVP', 'WSMVPs')
    
    grouped_cy_young_df = create_grouped_award_category_df(df, 'Cy Young Award', 'CyYoungs')
    
    grouped_gold_glove_df = create_grouped_award_category_df(df, 'Gold Glove', 'GoldGloves')
    
    grouped_all_star_mvp_df = create_grouped_award_category_df(df, 'All-Star Game MVP', 'ASGMVPs')
    
    grouped_rolaids_relief_df = create_grouped_award_category_df(df, 'Rolaids Relief Man Award', 'RolaidsReliefManAwards')
    ...
    grouped_silver_slugger_df = create_grouped_award_category_df(df, 'Silver Slugger', 'SilverSluggers')
    
    # merge these
    
    int1_df = pd.merge(grouped_pitching_triple_crown_df, grouped_triple_crown_df, on="playerID", how="outer")
