def create_grouped_pitching_df(df, isPostSeason=False):    
    grouped_df = df[['playerID', 'W', 'L', 'G', 'GS', 'CG', 'SHO', 'SV', 'IPouts', 'H', 'ER', 'HR', 'BB', 'SO', 'BAOpp', 'IBB', 'WP', 'HBP', 'BK', 'BFP', 'GF', 'R', 'SH', 'SF', 'GIDP']].groupby(['playerID'], dropna=False, as_index=False).sum()
