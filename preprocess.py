...
def get_df_for_modeling():
    ...
    grouped_hof_df = hof_df.loc[(hof_df['category'] == 'Player') & (hof_df['inducted'] == 'Y')][['playerID', 'inducted']]
    ...
    first_merge_df = pd.merge(grouped_allstar_df, grouped_reg_batting_df, on="playerID", how="outer")
    second_merge_df = pd.merge(first_merge_df, grouped_post_batting_df, on="playerID", how="outer")
    third_merge_df = pd.merge(second_merge_df, grouped_hof_df, on="playerID", how="outer")
